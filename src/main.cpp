...
#define REWARD_CUT_BLOCK 21000000
#if defined(NDEBUG)
# error "Yatracoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbb3e3b04c46d7f1da417a483921847e440133f9c13a4ee340185800b51f950a7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Yatracoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Yatracoin Signed Message:\n";
...
{
    // Yatracoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    //nSubsidy >>= (nHeight / 840000); // Yatracoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 30; // Yatracoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Yatracoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Yatracoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Yatracoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbd3049550b9ee773ddd02ca5c5b4000c3259086e29e4f89ccdf0726cff943db4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Yatracoin is created by Cosignys Limited in October 2018.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0403c84b0227951b8e1c23497eef900dc74e3471316b0387fac2ef041ad6bb4504e7198f8ccad1d8941bd42a399801af127a48ce6ccad39a7a8afbc0f63612d86c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1540337564;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1208567;

        if (fTestNet)
        {
            block.nTime    = 1540337580;
            block.nNonce   = 289585;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x00aa690658238389eeba861c52a53d751738d6e8d3490b3ea87d16c932219f98"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // Yatracoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YatracoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YatracoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static YatracoinMiner(CWallet *pwallet)
{
    printf("YatracoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("yatracoin-miner");
    ...
    try { loop {
        ...
        printf("Running YatracoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("YatracoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YatracoinMiner, pwallet));
}
